...
<div in:fly="{{ y: -100, duration: 1000 }}" class="flex border-b border-purple-600 pb-3 -sm:w-min -sm:border-r shadow-lg rounded-lg -sm:hidden" out:fly="{{ y: -100, duration: 1000 }}">
    ...
    <div class="flex gap-14 flex-row justify-items-center mt-4 -sm:hidden">
        ...
        <div transition:fly="{{ y: -100, duration: 250 }}"><ThemeSwitch /></div>
        {/if}
    </div>
</div>
<div transition:fly="{{ y: -100, duration: 1000 }}" class="sm:hidden border-b border-purple-600 grid grid-cols-4 grid-rows-2 justify-items-center place-items-center">
    {#if button}
